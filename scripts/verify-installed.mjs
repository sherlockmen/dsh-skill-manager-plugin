...
try {
  ...
  try {
    ...
    const preview = await invoke('evaluationPreviewInput', { request: { filename: 'acceptance.xlsx', input } })
    assert.equal(preview.sheets[0].count, 3)
    const direct = await invoke('evaluationCreate', { operationId: 'verify-direct-xlsx', request: { skillId: saved.skill.skillId, name: 'Direct workbook', sourceInput: { kind: 'excel', filename: 'acceptance.xlsx', input, sheet: '验收记录', expectedColumn: '金额' } } })
    assert.equal(direct.evaluation.name, 'Direct workbook')
    assert.equal(direct.evaluation.cases.length, 3)
    assert.deepEqual(direct.evaluation.cases[0].input, { '订单号': 'ORDER-1' })
    assert.equal(direct.evaluation.cases[0].expected, 20)
    const renamed = await invoke('evaluationRename', { operationId: 'verify-rename', request: { evaluationId: direct.evaluation.evaluationId, name: 'September acceptance' } })
    assert.equal(renamed.evaluation.name, 'September acceptance')
    assert.equal(renamed.evaluation.snapshotHash, direct.evaluation.snapshotHash)
    assert.deepEqual(renamed.evaluation.cases, direct.evaluation.cases)
    const textBatch = await invoke('evaluationCreate', { operationId: 'verify-direct-text', request: { skillId: saved.skill.skillId, name: 'Text acceptance', sourceInput: { kind: 'text', text: 'A plain business record.' } } })
    assert.equal(textBatch.evaluation.cases[0].input, 'A plain business record.')
