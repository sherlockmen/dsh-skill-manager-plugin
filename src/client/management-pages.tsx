...
function Action({ children, primary, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement> & { primary?: boolean }): React.ReactElement {
  return <ActionButton className={`sm-button gate-button ${primary ? 'sm-button-primary' : 'sm-button-secondary'}`} {...props}>{children}</ActionButton>
}
...
export function ScenariosPage({ initialScenarioId, initialSkillId, api, refresh, onChanged, onNotice, onDirtyChange }: { initialScenarioId?: string; initialSkillId?: string; api: RemoteApi; refresh: number; onChanged: () => void; onNotice: Notice; onDirtyChange?: (dirty: boolean) => void }): React.ReactElement {
  ...
  useEffect(() => {
    ...
    if (initialScenarioId === NEW_SCENARIO) {
      const next = { ...scenarioForm(), skillIds: initialSkillId ? [initialSkillId] : [] }
      setForm(next); setBaseline(pretty(next))
    }
  }, [initialScenarioId, initialSkillId])
  ...
  return <div className="sm-page management-page">
    ...
    <fieldset className="management-form-guard" disabled={busy}><div className="management-workspace">
      ...
      <main className="management-content"><ErrorBox message={detail.error} onRetry={detail.reload} />{detail.loading && selectedId !== NEW_SCENARIO ? <p className="management-empty">读取场景详情…</p> : null}{form ? <>
        <form onSubmit={storeForm}><section className="management-section"><div className="management-section-head"><div><h2>{form.scenarioId ? '场景设置' : '创建场景'}</h2><p>{formDirty ? '尚有未保存修改' : '已保存的设置用于后续测评与发布门禁。'}</p></div><Action type="submit" primary disabled={!api || busy || (!formDirty && Boolean(form.scenarioId))}>{save.busy ? '保存中…' : '保存场景'}</Action></div><div className="management-form-grid"><Field label="场景名称"><input required maxLength={500} value={form.name} onChange={event => update('name', event.target.value)} /></Field><Field label="区域口径"><WorkbenchSelect value={form.region} disabled={Boolean(current?.rules.length)} onChange={event => update('region', event.target.value)}>{Object.entries(regionLabel).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</WorkbenchSelect></Field><Field label="状态"><WorkbenchSelect value={form.status} onChange={event => update('status', event.target.value)}><option value="draft">草稿</option><option value="active">活动</option><option value="archived">已归档</option></WorkbenchSelect></Field><Field label="最低有效标注数"><input required type="number" min="1" max="100000" step="1" value={form.minimumLabeledCases} onChange={event => update('minimumLabeledCases', event.target.value)} /></Field><Field label="最低准确率（%）"><input required type="number" min="0" max="100" step="0.1" value={form.minimumAccuracy} onChange={event => update('minimumAccuracy', event.target.value)} /></Field><Field label="场景说明" wide><textarea rows={3} value={form.description} onChange={event => update('description', event.target.value)} /></Field></div></section>
        ...
        {ruleDraft ? <div className="management-rule-editor">
          ...
          {ruleDraft.layouts.map((layout: AnyRecord, index: number) => <fieldset className="management-layout" key={layout.layoutId}>
            ...
            <div className="management-form-grid">
              ...
              <Field label="记录边界"><WorkbenchSelect value={layout.recordMode || 'row'} onChange={event => patchLayout(index, { recordMode: event.target.value })}><option value="row">一行一条记录</option><option value="group">按字段分组</option><option value="sheet">整表一条记录</option></WorkbenchSelect></Field>
            ...
            <div className="management-table-scroll"><table><thead><tr><th>原始表头</th><th>映射到 Canonical 字段</th></tr></thead><tbody>{layout.headers.map((header: string) => <tr key={header}><td>{header}</td><td><WorkbenchSelect aria-label={`${layout.layoutId} ${header} 字段映射`} value={layout.mapping[header] || ''} onChange={event => patchLayout(index, { mapping: { ...layout.mapping, [header]: event.target.value } })}><option value="">请选择</option>{ruleDraft.canonicalFields.map((field: string) => <option key={field} value={field}>{field}</option>)}</WorkbenchSelect></td></tr>)}</tbody></table></div>
          </fieldset>)}
          ...
          <div className="management-actions"><Action primary disabled={!api || busy || formDirty || !ruleDirty} onClick={() => saveRule()}>保存规则候选</Action><Action disabled={!api || busy || dirty || ruleDraft.status !== 'candidate'} onClick={() => confirm(false)}>确认规则</Action><Action disabled={!api || busy || dirty || ruleDraft.status !== 'confirmed'} onClick={() => confirm(true)}>回归并发布</Action><span>{ruleDirty ? '规则修改尚未保存' : labels[ruleDraft.status]}</span></div>
        </div> : null}
...
export function SettingsPage({ api, onExit, onNotice, onDirtyChange }: { api: RemoteApi; onExit: () => void; onNotice: Notice; onDirtyChange?: (dirty: boolean) => void }): React.ReactElement {
  ...
  const traceDirty = days !== baseline
  const dirty = traceDirty || modelDirty
  const dirtyRef = useRef(dirty); dirtyRef.current = dirty
  useDirtyProtection(dirty, onDirtyChange)
  ...
  const leave = () => {
    ...
    if (onDirtyChange || !dirty || window.confirm('设置尚未保存，放弃修改并返回原生 Harness 吗？')) onExit()
  }
  const statusRows = [
  ...
  return <div className="sm-page management-page">
    <PageHead title="系统设置" description="选择 DSH 模型、查看服务状态、管理 Trace 保留策略和审计记录。"><Action onClick={() => { health.reload(); settings.reload(); audit.reload() }} disabled={health.loading}>刷新状态</Action><Action onClick={leave}>返回原生 Harness</Action></PageHead>
